...
func (f StringField) writeTo(b *bytes.Buffer) error {
	sLen := len(f.Value)
	if err := binary.Write(b, binary.LittleEndian, int32(sLen)); err != nil {
		return err
	}
...
func readDBValueFrom(b *bytes.Buffer, t DBType) (DBValue, error) {
	...
	case StringType:
		var sLen int32
		if err := binary.Read(b, binary.LittleEndian, &sLen); err != nil {
			return nil, err
		}

		readLen := max(sLen, int32(StringLength))
		buf := make([]byte, readLen)
		if err := binary.Read(b, binary.LittleEndian, &buf); err != nil {
			return nil, err
		}

		return &StringField{Value: string(buf[:sLen])}, nil
	default:
